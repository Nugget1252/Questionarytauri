...
/* ================================================================
   ...
   ================================================================ */
    function showPDF(url, customName = null) {
      ...
      // Resolve locally downloaded Electron path if available
      let finalUrl = url;
      if (window.DownloadManager && !url.startsWith('blob:') && !url.startsWith('data:')) {
        finalUrl = window.DownloadManager.resolveDocumentUrl(url);
      }

      if (typeof window.setCurrentPDF === 'function') {
        window.setCurrentPDF(finalUrl, filename);
      }

      if (pdfViewer) {
        // Resolve absolute URL for local files, custom Electron protocol, or keep raw blob: URL
        const targetUrl = (finalUrl.startsWith('blob:') || finalUrl.startsWith('data:') || finalUrl.startsWith('http') || finalUrl.startsWith('local-pdf:'))
        ? finalUrl
        : new URL(finalUrl, window.location.href).href;
      ...
      if (pdfViewerContainer) {
        ...
        if (typeof renderPdfBookmarks === 'function') renderPdfBookmarks(finalUrl);
        if (typeof window.setCurrentPdfForBookmarks === 'function') window.setCurrentPdfForBookmarks(finalUrl);
        else window.currentPdfUrlForBookmarks = finalUrl;
      }
