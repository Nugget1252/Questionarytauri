...
/* js/app.js - Questionary Core Application Engine */
...
if (window._HOT_APP_JS_LOADED && document.currentScript && !document.currentScript.id?.startsWith('hot-js-')) {
    ...
    // ================================================================
    // 1. LIVE GLOBAL SCOPE PATCHER (Evaluates module overrides)
    // ================================================================
        ...
        try {
            ...
            if (raw) {
                ...
                if (files['js/studyRoom.js']) {
                    ...
                    console.log('[HotUpdate] Evaluated latest js/studyRoom.js into global scope');
                }
                if (files['js/features.js']) {
                    window.eval(files['js/features.js']);
                    console.log('[HotUpdate] Evaluated latest js/features.js into global scope');
                }
            }
        } catch (err) {
    ...
    // ================================================================
    // 3. UI UTILITIES & SELECTION GUARD
    // ================================================================
    ...
    // ================================================================
    // 4. DIALOG SYSTEM (CONFIRM, PROMPT, INFO)
    // ================================================================
            ...
            overlay.innerHTML = `
                <div class="dialog-box">
                    ...
                    <div class="dialog-actions">
                        ...
                        <button class="dialog-btn dialog-btn-confirm" style="background: ${type === 'danger' ? 'var(--red, #ef4444)' : 'var(--accent)'}">${escapeHtml(confirmText)}</button>
                    </div>
                </div>
            `;
        ...
        return new Promise(resolve => {
            ...
            overlay.innerHTML = `
                <div class="dialog-box">
                    ...
                    <h3 class="dialog-title">${escapeHtml(title)}</h3>
                    <p class="dialog-message">${escapeHtml(message)}</p>
                    <input class="dialog-input" type="text" value="${escapeHtml(defaultValue)}" placeholder="${escapeHtml(placeholder)}" spellcheck="false" />
                    <div class="dialog-actions">
                        <button class="dialog-btn dialog-btn-cancel">${escapeHtml(cancelText)}</button>
        ...
        return new Promise(resolve => {
            ...
            overlay.innerHTML = `
                <div class="dialog-box">
                    ...
                    <h3 class="dialog-title">${escapeHtml(title)}</h3>
                    <div class="dialog-message dialog-message-scrollable">${escapeHtml(message).replace(/\n/g, '<br>')}</div>
                    <div class="dialog-actions">
                        <button class="dialog-btn dialog-btn-confirm">${escapeHtml(buttonText)}</button>
                    </div>
                </div>
            `;
    ...
    // ================================================================
    // 5. DUAL-ENGINE SQLITE & INDEXEDDB DATABASE SERVICE
    // ================================================================
    ...
    const DbService = {
        ...
        async init() {
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('DbService init timeout')), 2500)
            );
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (await this.isValidDatabase()) {
                            ...
                            console.log(`[SQLite] Loaded questionary.db from: ${p}`);
                            return true;
                        }
                    }
                } catch (fetchErr) {}
    ...
    window.resetDatabase = async function() {
        const ok = await showConfirm('Reset database cache? This will clear stored database nodes and reload.', { title: 'Reset Database', type: 'danger', confirmText: 'Reset' });
        if (ok) {
    ...
    // ================================================================
    // 6. FAVORITES & RECENT STORAGE (TAURI & LOCALSTORAGE)
    // ================================================================
    ...
    // ================================================================
    // 7. THEME & ACCESSIBILITY SUBSYSTEM (DELEGATED)
    // ================================================================
    ...
    // Unified Delegated Event Handler for Header Toggles and Global Menus
    document.addEventListener('click', (e) => {
        ...
        const userBadge = e.target.closest('#userBadge, .user-badge');
        if (userBadge && !e.target.closest('.user-dropdown-menu')) {
            e.preventDefault();
            e.stopPropagation();
        ...
        // Close User Badge Dropdown on Outside Click
        ...
        // Code Cache Reset Actions
        const resetBtn = e.target.closest('#resetCodeCacheBtn, #resetCodeCacheBtnSettings, .reset-cache-btn, #homeSyncBtn');
        if (resetBtn) {
            e.preventDefault();
            e.stopPropagation();
            const doReset = () => {
            ...
            showConfirm('Clear stored updates and re-sync directly from GitHub?', {
    ...
    function setupAccessibilityToggles() {
        const map = [
            { id: 'highContrastToggle', key: 'highContrast', css: 'high-contrast' },
            { id: 'largeTextToggle', key: 'largeText', css: 'large-text' },
            { id: 'reducedMotionToggle', key: 'reducedMotion', css: 'reduced-motion' },
            { id: 'enhancedFocusToggle', key: 'enhancedFocus', css: 'enhanced-focus' }
        ];

        map.forEach(({ id, key, css }) => {
            ...
            btn.addEventListener('click', (e) => {
                ...
                localStorage.setItem('accessibility-' + key.replace(/([A-Z])/g, '-$1').toLowerCase(), accessibilitySettings[key]);
                updateAccessibilityToggleStates();
                document.body.classList.toggle(css, accessibilitySettings[key]);
            });
        });
        updateAccessibilityToggleStates();
    }
    ...
    // ================================================================
    // 8. NAVIGATION, VIEWS & BREADCRUMBS
    // ================================================================
        ...
        if (settings.rememberLocation !== false) {
            ...
            if (lastView && lastView !== 'home') {
                ...
                const navMap = {
                    ...
                    'analytics': 'analyticsNav', 'planner': 'studyPlannerNav', 'flashcards': 'flashcardsNav',
        ...
        items.forEach(item => {
            ...
            tile.innerHTML = `
                <div class="tile-top-bar">
                    ...
                    ${isImportedPdf ? `<button onclick="event.stopPropagation(); moveDocumentItemToLibrary('${escapeHtml(key)}')" title="Move to Library"><i class="fas fa-book-open"></i></button>` : ''}
                </div>
                <div class="tile-icon">
                    <i class="fas ${isFolder ? 'fa-folder' : (key.match(/\.(png|jpg|jpeg|webp|gif|svg)$/i) ? 'fa-image' : 'fa-file-pdf')}"></i>
                </div>
    ...
    window.getCurrentDocumentsLevel = async function() {
        return await DbService.getChildren(path);
    };
    window.getCurrentLevel = window.getCurrentDocumentsLevel;
    ...
    function showView(viewName) {
        ...
        const sections = {
            home: document.getElementById('tilesSection'),
            ...
            flashcards: document.getElementById('flashcardsSection'),
            ...
            studyRoom: document.getElementById('studyRoomSection')
        };
        ...
        switch (viewName) {
            case 'home':
                ...
                break;
            case 'favorites':
                if (sections.favorites) sections.favorites.style.display = 'block';
                renderFavorites();
                break;
            case 'recent':
                ...
                break;
            case 'analytics':
                if (sections.analytics) sections.analytics.style.display = 'block';
                renderAnalytics();
                break;
            case 'planner':
                if (sections.planner) sections.planner.style.display = 'block';
                renderCalendar();
                renderSessions();
                break;
            case 'flashcards':
                if (sections.flashcards) sections.flashcards.style.display = 'block';
                renderFlashcardDecks();
                break;
            case 'notes':
                if (sections.notes) sections.notes.style.display = 'block';
                renderNotes();
                break;
            case 'progress':
                if (sections.progress) sections.progress.style.display = 'block';
                break;
            case 'reminders':
                if (sections.reminders) sections.reminders.style.display = 'block';
            ...
            case 'settings':
                ...
                break;
            case 'studyRoom':
                if (sections.studyRoom) sections.studyRoom.style.display = 'block';
                if (typeof window.renderStudyRoom === 'function') window.renderStudyRoom();
                break;
            case 'tags':
                if (sections.tags) sections.tags.style.display = 'block';
    ...
    function closePDF() {
        ...
        window.clearCurrentPDF();
    }
    window.closePDF = closePDF;

    var _currentImageBlobUrl = null;
    ...
    // ================================================================
    // 10. SEARCH & RECENT ACTIVITY ENGINE
    // ================================================================
                    ...
                    return `
                        <div class="search-result-item" onclick="${onclickHandler}">
                            ...
                            <div class="search-result-info">
                                ...
                                <span class="search-result-path">${escapeHtml(r.path.join(' > '))}</span>
                            </div>
                        </div>
                    `;
                }).join('');
    ...
    function navigateToNote(noteId) {
        ...
        document.getElementById('globalSearch')?.setAttribute('value', '');
        showView('notes');
    ...
    function navigateToFlashcard(deckId) {
        ...
        setTimeout(() => { startStudyDeck(deckId); }, 100);
    }
        ...
        container.innerHTML = sorted.map(session => `
            <div class="session-item">
                <div class="session-info">
                    ...
                    <span>${escapeHtml(session.date)} at ${escapeHtml(session.time)}</span>
                </div>
    ...
    function openSessionModal() {
        ...
        if (!modal) return;
        const subj = document.getElementById('sessionSubject');
        const dt = document.getElementById('sessionDate');
        const tm = document.getElementById('sessionTime');
        if (subj) subj.value = '';
    ...
    function deleteSession(sessionId) {
        showConfirmModal('Delete Session', 'Are you sure you want to delete this study session?', () => {
        ...
        container.innerHTML = quickLinks.map(ql => {
            ...
            return `
                <div class="quick-link-item" data-id="${ql.id}" data-path="${ql.pathArray.join('|')}" data-is-file="${isFile}" data-url="${ql.url || ''}">
                    ...
                    <span class="quick-link-name">${escapeHtml(ql.name || ql.pathArray[ql.pathArray.length - 1] || 'Unknown')}</span>
            ...
            <div class="quicklink-choice-dialog">
                ...
                <div class="quicklink-choice-options">
                    <button class="quicklink-choice-btn" id="addPdfQuickLink">
                        ...
                        <small>${escapeHtml(pdfName)}</small>
                    </button>
                    ${path.length > 0 ? `
    ...
    // ================================================================
    // 16. TIMER SUBSYSTEM (PHYSICS DRAG, RESIZE, PRESETS, LAPS)
    // ================================================================
    function initializeTimer() {
        ...
        if (timerReopenBtn && !timerReopenBtn.dataset.bound) {
            timerReopenBtn.dataset.bound = 'true';
            timerReopenBtn.addEventListener('click', () => {
                showTimer();
        ...
        timerPresets.forEach(btn => {
            ...
            btn.dataset.bound = 'true';
            btn.addEventListener('click', () => { selectTimerPreset(btn, duration); });
        });

        if (timerClose && !timerClose.dataset.bound) {
            timerClose.dataset.bound = 'true';
            timerClose.addEventListener('click', () => hideTimer());
        }

        if (timerMinimize && !timerMinimize.dataset.bound) {
            timerMinimize.dataset.bound = 'true';
            timerMinimize.addEventListener('click', () => toggleTimerMinimize());
        }

        if (timerStart && !timerStart.dataset.bound) {
            timerStart.dataset.bound = 'true';
            timerStart.addEventListener('click', () => startTimer());
        }

        if (timerPause && !timerPause.dataset.bound) {
            timerPause.dataset.bound = 'true';
            timerPause.addEventListener('click', () => pauseTimer());
        }

        if (timerResume && !timerResume.dataset.bound) {
            timerResume.dataset.bound = 'true';
            timerResume.addEventListener('click', () => resumeTimer());
        }

        if (timerReset && !timerReset.dataset.bound) {
            timerReset.dataset.bound = 'true';
            timerReset.addEventListener('click', () => resetTimer());
        }

        if (timerLap && !timerLap.dataset.bound) {
            timerLap.dataset.bound = 'true';
            timerLap.addEventListener('click', () => addLap());
        }

        if (timerMiniLap && !timerMiniLap.dataset.bound) {
            timerMiniLap.dataset.bound = 'true';
            timerMiniLap.addEventListener('click', () => addLap());
        }
    }
    ...
    function initializeTimerDrag() {
        ...
        if (!timerPanel || !dragHandle || dragHandle.dataset.dragInit === 'true') return;
        dragHandle.dataset.dragInit = 'true';
    ...
    function initializeTimerResize() {
        ...
        if (!timerPanel || !resizeHandle || resizeHandle.dataset.resizeInit === 'true') return;
        resizeHandle.dataset.resizeInit = 'true';
    ...
    function toggleTimerMinimize() {
        ...
        if (timerControls && isMinimized) timerControls.style.display = 'flex';
    ...
    function timerFinished() {
        ...
        showNotification('⏰ Timer Complete! Time is up!', 'success');
    ...
    function addCustomPreset() {
        ...
        if (!label) { showNotification('Please enter a label', 'error'); return; }
        if (!minutes || isNaN(minutes) || minutes < 1) { showNotification('Please enter a valid duration (minutes)', 'error'); return; }
    ...
    function removeCustomPreset(id) {
        ...
        localStorage.setItem('questionary-timer-presets', JSON.stringify(customTimerPresets));
        document.querySelector(`[data-preset-id="${id}"]`)?.remove();
    ...
    // ================================================================
    // 17. TAGGING, BOOKMARKS, QUEUE & SHARING
    // ================================================================
    ...
    window.filterByTagHome = filterByTagHome;

    function addPageBookmark(docPath, pageNumber, label = '') {
        if (!pageBookmarks[docPath]) pageBookmarks[docPath] = [];
        const existing = pageBookmarks[docPath].find(b => b.page === pageNumber);
        if (existing) { showNotification('Page already bookmarked', 'info'); return; }
    ...
    function resetKeybinds() {
        ...
        showNotification('Keyboard shortcuts reset to defaults.', 'info');
        return binds;
    }
    ...
    function renderKeybindsSettings() {
        ...
        for (const [cat, ids] of Object.entries(categories)) {
            ...
            for (const id of ids) {
                ...
                html += `
                    <div class="kb-row">
                        <span class="kb-action">${escapeHtml(b.label)}</span>
                        <button class="kb-key-btn" data-bind-id="${id}" title="Click to change">${escapeHtml(formatKeybindDisplay(b))}</button>
                    </div>
                `;
            }
        }
    ...
    function startKeybindCapture(btn) {
        ...
        function onKey(e) {
            ...
            _capturingBind = null;
            showNotification(`Shortcut updated: ${binds[bindId].label}`, 'success');
        }
        document.addEventListener('keydown', onKey, true);
    }

    // Consolidated Master Keydown Listener
    document.addEventListener('keydown', async (e) => {
        const isInputFocused = e.target.closest('input, textarea, [contenteditable="true"]');
        ...
        if (keybindMatches(e, 'goBack') && path.length > 0) {
            e.preventDefault();
            await handleBackButton();
            return;
        }

        if (keybindMatches(e, 'navBack')) {
            e.preventDefault();
            const pdfViewerContainer = document.getElementById('pdfViewerContainer');
            if (pdfViewerContainer && pdfViewerContainer.style.display !== 'none') {
                closePDF();
    ...
    function loadSettingsState() {
        ...
        if (verticalNavToggle) {
            ...
            document.body.classList.toggle('vertical-navbar-mode', !!settings.verticalNavbar);
        }
        ...
        if (animationsToggle) {
            ...
            document.body.classList.toggle('reduced-animations', settings.animations === false);
        }
    ...
    function setupSettingsToggles() {
        ...
        setupToggleRow('verticalNavbarToggle', (checked) => {
            document.body.classList.toggle('vertical-navbar-mode', checked);
            if (!checked) closeSidebar();
            showNotification(checked ? 'Vertical navbar enabled' : 'Horizontal navbar restored', 'info');
        });
        ...
        setupToggleRow('rememberLocationToggle', (checked) => {
            showNotification(checked ? 'Will remember last location' : 'Will start at home on launch', 'info');
        });
    }
    ...
    function setupSettingsActions() {
        ...
        if (clearDataBtn && !clearDataBtn.dataset.bound) {
            ...
            clearDataBtn.addEventListener('click', (e) => {
                ...
                showConfirmModal(
                    ...
                    async () => {
                        const keysToKeep = ['revamp-dpsnt-remember', 'revamp-dpsnt-session'];
        ...
        if (openSettingsBtn && !openSettingsBtn.dataset.bound) {
    ...
    function checkSavedLogin() {
        ...
        if (savedLogin) {
            try {
                ...
                showApp();
                initializeAppAfterLogin();
                setTimeout(() => showNotification(`Welcome back, ${currentUser.username}!`, 'success'), 300);
                return true;
            } catch (e) {
                localStorage.removeItem('revamp-dpsnt-remember');
            }
    ...
    // ================================================================
    // 21. MOBILE & RESPONSIVE DRAWER NAVIGATION
    // ================================================================
    ...
    async function checkForUpdatesManual() {
        ...
        if (window.__TAURI__ && (window.__TAURI__.updater || window.__TAURI_PLUGIN_UPDATER__)) {
            try {
                showNotification('Checking for native app updates...', 'info');
                ...
                console.warn('[Tauri Native Updater]: Native check skipped. Falling back to code updater:', tauriErr);
            }
        }

        if (window.hotCodeUpdater && typeof window.hotCodeUpdater.check === 'function') {
            try {
                showNotification('Checking for code updates...', 'info');
                ...
                if (updates && updates.length > 0) {
                    ...
                    showNotification('Application is completely up to date!', 'success');
                    resetUpdateButton();
                }
            } catch (hotErr) {
            ...
            if (!nativeUpdateFound) {
                showNotification('No updates found.', 'info');
                resetUpdateButton();
            }
        }
    ...
    function resetUpdateButton() {
        ...
        if (btn) {
            ...
            btn.classList.remove('update-available', 'downloading', 'restarting', 'checking');
        }
        const progressBar = document.getElementById('downloadProgressBar');
        if (progressBar) progressBar.style.display = 'none';
    ...
    // ================================================================
    // 23. COMPREHENSIVE INITIALIZATION ROUTINE
    // ================================================================
    function initializeNavigation() {
        const navBindings = [
            { id: 'homeNav', view: 'home', action: async () => { path = []; await navigateToPath([]); } },
            ...
            { id: 'notesNav', view: 'notes', action: () => { if (typeof renderVoiceNotesGrid === 'function') renderVoiceNotesGrid(); } },
            { id: 'tagsNav', view: 'tags', action: () => { if (typeof renderTagsMain === 'function') renderTagsMain(); if (typeof renderTaggedItems === 'function') renderTaggedItems(); } },
            { id: 'progressNav', view: 'progress' },
            { id: 'remindersNav', view: 'reminders', action: () => { if (typeof window.renderReminders === 'function') window.renderReminders(); } },
            { id: 'studyRoomNav', view: 'studyRoom', action: () => { if (typeof window.renderStudyRoom === 'function') window.renderStudyRoom(); } },
            { id: 'settingsNav', view: 'settings', action: () => { if (typeof window.renderSettings === 'function') window.renderSettings(); } }
        ];

        navBindings.forEach(({ id, view, action }) => {
    ...
    function initializeViewControls() {
        ...
        if (gridViewBtn && !gridViewBtn.dataset.bound) {
            gridViewBtn.dataset.bound = 'true';
        ...
        if (listViewBtn && !listViewBtn.dataset.bound) {
            listViewBtn.dataset.bound = 'true';
    ...
    function initializeNewFeatures() {
        ...
        // Study Flashcard Modal Handlers
        ...
        if (closeStudyModalBtn) closeStudyModalBtn.onclick = closeStudyModal;
    ...
    async function initializeApp() {
        console.log('[App] Starting Questionary core initialization...');
        ...
        if (checkUpdatesBtn && !checkUpdatesBtn.dataset.bound) {
            checkUpdatesBtn.dataset.bound = 'true';
        ...
        console.log('[App] Questionary application initialized successfully.');
    }
